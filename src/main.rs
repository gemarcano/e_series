...
fn find_closest_e(desired: f64, e_series: &[f64]) -> f64 {
    let decade = 10f64.powf(desired.log10().floor());
    let mut closest = 0.0;
    let mut last = decade * 10.0;
    for i in e_series {
        let current = decade * i;
        if (current - desired).abs() < last {
            last = (current - desired).abs();
            closest = current;
        }
    }
    // FIXME this isn't quite right, rounding should happen... elsewhere?
    closest.round()
}

fn find_next_e(desired: f64, e_series: &[f64]) -> f64 {
    let decade = 10f64.powf(desired.log10().floor());
    let mut closest = 0.0;
    let mut last = decade * 10.0;
    for i in e_series {
        let current = decade * i;
        if (current - desired) >= 0.0 && (current - desired) < last {
...
impl fmt::Display for ParseError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "failed to parse: {}", self.reason)
    }
}

fn parse_positive_f64(number: &str) -> Result<f64, ParseError> {
    let number = number.parse::<f64>().map_err(|_| ParseError {
        reason: "could not parse argument",
    })?;
    if number < 0.0 {
        return Err(ParseError {
            reason: "argument must not be negative",
        });
    }
    Ok(number)
}
...
enum Commands {
    /// Compute the closes E-series value
    Closest {
        /// Value to find the closest E-series value of
        #[arg(value_parser = parse_positive_f64)]
        value: f64,
    },
    /// Compute the next/following E-series value
    Next {
        /// Value to find the next E-series value of
        value: f64,
    },
}
...
struct Cli {
    #[command(subcommand)]
    command: Commands,
    #[arg(value_enum, default_value_t=Eseries::E24, short, long)]
    series: Eseries,
}

fn main() {
    let args = Cli::parse();
    let series: &[f64] = match args.series {
        Eseries::E3 => &E3,
        Eseries::E6 => &E6,
        Eseries::E12 => &E12,
        Eseries::E24 => &E24,
    ...
    let result = match args.command {
        Commands::Closest { value } => find_closest_e(value, series),
        Commands::Next { value } => find_next_e(value, series),
    };
    println!("{result}");
}
